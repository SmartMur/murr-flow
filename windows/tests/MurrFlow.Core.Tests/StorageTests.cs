using MurrFlow.Core;
using MurrFlow.Dictionary;
using Shouldly;
using Xunit;

namespace MurrFlow.CoreTests;
...
{
    private readonly string _path = Path.Combine(Path.GetTempPath(), $"murr-flow-dict-{Guid.NewGuid():N}.txt");
...
{
    private readonly string _path = Path.Combine(Path.GetTempPath(), $"murr-flow-hist-{Guid.NewGuid():N}.jsonl");
...
{
    private readonly string _path = Path.Combine(Path.GetTempPath(), $"murr-flow-set-{Guid.NewGuid():N}.json");
