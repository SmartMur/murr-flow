# Murr-flow for Windows

The Windows port of Murr-flow — push-to-talk dictation, on-device.
...
├─ global.json                    SDK pin
├─ src/
│  ├─ MurrFlow.Dictionary/          corrections + biasing          net10.0
│  ├─ MurrFlow.Abstractions/        the four platform interfaces   net10.0
│  ├─ MurrFlow.Core/                engine, segmenter, storage     net10.0
│  ├─ MurrFlow.Speech/              Parakeet via sherpa-onnx       net10.0
│  ├─ MurrFlow.Testing/             fakes for the interfaces       net10.0
│  ├─ MurrFlow.App/                 Avalonia UI                    net10.0
│  └─ MurrFlow.Platform.Windows/    the ONLY Win32 code            net10.0-windows
└─ tests/
   ├─ MurrFlow.Dictionary.Tests/    the shared vectors             24 tests
   ├─ MurrFlow.Core.Tests/          engine, chunking, storage      26 tests
   └─ MurrFlow.App.Tests/           headless Avalonia UI           13 tests
```
...
`MurrFlow.App` loads the platform layer **by reflection** rather than referencing it. A direct
...
```bash
cd windows
dotnet build MurrFlow.sln --no-incremental -warnaserror
dotnet test  MurrFlow.sln
```

**On macOS or Linux** — use the solution filter. `MurrFlow.Platform.Windows` targets
...
```bash
cd windows
dotnet test MurrFlow.CrossPlatform.slnf -c Release      # ~0.5s, 63 tests
```
